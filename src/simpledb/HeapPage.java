...
public class HeapPage implements Page {
	...
	public Tuple getTuple(int entryID) {
		if(entryID<entryCount()) { // first check if entryID is in range on entryCount
			
			// get location of tuple, use create tuple method and set its record id
			int location=tupleLocation(entryID);		
			if(location==-1) { //tuple is deleted
				return null;
			}
			DataInputStream in = new DataInputStream(new ByteArrayInputStream(data, location, data.length-location));
			Tuple tuple= createTuple(in);
	...
	public Iterator<Tuple> iterator() {  
		ArrayList<Tuple> tupleList= new ArrayList<>(); // store all tuples in arraylist and use arraylist default iterator
	    for(int j=0;j< entryCount();j++) {
          	if(getTuple(j)!=null) {
          		tupleList.add(getTuple(j));
          	}
	...
	public void deleteTuple(Tuple t) throws DbException {
		...
		PageId p = t.getRecordId().getPageId(); // get tuple page ID
			
		if(!p.equals(this.pid)) // checks weather the tuple is on the given page or not 
		{
			throw new DbException("find tuple on other page it's not here");
			
		}
		
		if(this.pid == null)
		{
			throw new DbException("Tupple is already empty");
		}
		
		if(t==null || t.getRecordId()==null ||getTuple(t.getRecordId().tupleno) ==null) {
			throw new DbException("Tuple doesnt exist"); // check if tuple exists first
		}
		saveEntryCount(entryCount()-1); // decrement entryCount on page
		saveTupleLocation(t.getRecordId().tupleno, -1); // set location to -1 for this tuple
	...
	public void addTuple(Tuple t) throws DbException {
		...
		int space=b.length+4;
		if(freeSpaceSize()<space) {
			throw new DbException("No space to add tuple");
		}
		else {
			int location=endOfFreeSpace()-b.length;
			System.arraycopy(b, 0, data,location , b.length); // add b[] to data at end of free space
			saveEntryCount(entryCount()+1);  // increment entryCount
			saveTupleLocation(t.getRecordId().tupleno, location-4); // save location of tuple in header
			t.setRecordId(new RecordId(pid, entryCount())); // set record id
