	...
	public Page getPage(TransactionId tid, PageId pid, Permissions perm)
		...
		if(pid!=null) {
			...
			if( id2page.containsKey(pid)) {
				page = id2page.get(pid);	 // if page is present in id2page return it
				return page;
			}
			else  {
			 if(numPages>=id2page.size()) { // if page is not present check if there is room for this request else throw exception
				 ...
				 throw new DbException("Can only serve "+numPages+ " requests");
			 }
