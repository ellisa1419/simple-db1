...
public class TupleDesc {
	...
	public String toString() {
		// some code goes here
		String tupleDescValue="";
		if(types.length==names.length) { // for every table fields in schema will be same as type in actual data
			for(int i =0; i< types.length;i++) {
				tupleDescValue+= types[i].name()+"("+names[i]+")";
				tupleDescValue+= "|";
			}
			tupleDescValue=tupleDescValue.substring(0, tupleDescValue.length()-1);
			
			return tupleDescValue;
		}
		else {
			throw new UnsupportedOperationException("Implement this");
		}
