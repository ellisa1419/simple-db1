...
public class Tuple {
	...
	public String toString() {
		// some code goes here
		String tupleValue="";
		for(Field field:fields) { // tuple has fields, check for both int and String and generate toString method
			if(field instanceof IntField) {
				tupleValue+= ((IntField) field).toString();
				tupleValue+= "|";
			}
			if(field instanceof StringField) {
				tupleValue+=((StringField) field).toString();
				tupleValue+= "|";
			}
		}
		tupleValue=tupleValue.substring(0, tupleValue.length()-1);
		return tupleValue;
	}
}
